...
- **Password flow - testing only:** You can configure the client to use the legacy password flow. Username and credentials are set via environment variables or arguments to the `Client` constructor. This method is practical to use for testing but your IdP will probably (and rightfully so) not allow it in production.
- **Device flow:** By default, the Lomas client chooses the device flow. It prints out a device verification url embedding a unique user code which the user accesses to approve the device after authenticating themselves. This is the preferred method for receiving an access token to pass along to the server.
...
**Note if using Dex:** By default, the lifetime of tokens delivered by Dex is set very short to speed up our test runs. Make sure to change the setting (in `devenv/dex.nix`) to larger value when testing the dashboard.
