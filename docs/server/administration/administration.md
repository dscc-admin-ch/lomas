...
If you are deploying with Kubernetes, ensure that the ``dashboard.enabled`` value is set to ``true`` in your values.yaml.
If ``dashboard.enabled`` is not set to ``true``, the server will only be accessible though the API, and the dashboard will not be available.
