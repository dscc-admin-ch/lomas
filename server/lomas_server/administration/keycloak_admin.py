...
def get_kc_admin(kc_config: KeycloakClientConfig) -> KeycloakAdmin:
    ...
    """
    return KeycloakAdmin.from_client_credentials(
...
def add_kc_user(
    ...
    try:
        ...
        user_service_account_uid = kc_admin.clients(user_client_uid).service_account_user.get()["id"]
...
def del_kc_user(kc_config: KeycloakClientConfig, user: str) -> None:
    ...
    # Delete user
    user_id = kc_admin.users.get(username=user)[0]["id"]
...
def del_all_kc_users(kc_config: KeycloakClientConfig) -> None:
    ...
    for client in clients:
        if client.get("attributes", {}).get(KCAttributeNames.LOMAS_USER_CLIENT):
            client_id = client["id"]
